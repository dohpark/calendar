...
describe('달력 단위 변경', () => {
  ...
  test('달력 단위 선택 버튼을 클릭 시 드랍다운이 화면에 나타난다.', async () => {
    ...
    const calendarViewUnitList = screen.getByRole('listbox');
    expect(calendarViewUnitList).toBeVisible();

    const { getAllByRole } = within(calendarViewUnitList);
    const calendarViewUnitItem = getAllByRole('listitem');
    expect(calendarViewUnitItem.length).toEqual(['일', '주', '월']);
  });
  ...
  test('드랍다운에서 일을 변경을 하면 오늘의 날짜로 변경된다.', async () => {
    ...
    const calendarViewUnitDay = screen.getByRole('listitem', { name: /일/ });
...
describe('왼쪽 및 오른쪽 버튼을 통한 선택 월 / 주 / 일 변경', () => {
